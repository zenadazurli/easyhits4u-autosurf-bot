#!/usr/bin/env python3
# app.py - Login con BQL + forzatura sesids + Autosurf con requests
...
def do_login(api_key):
    ...
    # 🔧 FORZATURA: visita dashboard e /surf/ per ottenere sesids
    log("   🔄 Forzatura recupero sesids...")
    dashboard = session.get("https://www.easyhits4u.com/member/", verify=False, timeout=15)
    log(f"   Dashboard status: {dashboard.status_code}")
    time.sleep(2)
    surf_page = session.get("https://www.easyhits4u.com/surf/", verify=False, timeout=15)
    log(f"   Surf page status: {surf_page.status_code}")
    time.sleep(2)
    ...
    if 'user_id' in final_cookies and 'sesids' in final_cookies:
        ...
        log(f"   🍪 Cookie disponibili: {list(final_cookies.keys())}")
        return None
...
def main():
    log("=" * 50)
    log("🚀 LOGIN + AUTOSURF CON BQL (forzatura sesids)")
    log("=" * 50)
